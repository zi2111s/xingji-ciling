...
for (const line of lines) {
    ...
    if (match) {
        ...
        // 更智能的词性提取逻辑：支持多个词性，如 "ad. &prep."
        const possiblePosPattern = /^([a-z.]+\s*(?:&|;|,)?\s*[a-z.]*)\s*(.*)$/i;
        let posMatch = explanation.match(possiblePosPattern);
        if (posMatch) {
            // 进一步验证：如果提取的部分看起来确实像词性
            const candidatePos = posMatch[1].trim();
            const posPartCount = candidatePos.split(/[&;,]/).filter(p => p.trim()).length;
            if (posPartCount > 0 && candidatePos.match(/^[a-z.&;, ]+$/i)) {
                partOfSpeech = candidatePos;
                meaning = posMatch[2].trim();
                
                // 继续检查：如果意思开头还有类似词性的部分，也提取出来
                while (true) {
                    const nestedPattern = /^([a-z.]+\s*(?:&|;|,)?\s*[a-z.]*)\s*(.*)$/i;
                    const nestedMatch = meaning.match(nestedPattern);
                    if (nestedMatch) {
                        const nestedCandidate = nestedMatch[1].trim();
                        if (nestedCandidate.match(/^[a-z.&;, ]+$/i)) {
                            // 如果是 "ad." 这种，通常是重复的，我们不添加，但如果是新的不同的就添加
                            // 简单处理：如果已经有了，就不重复添加，直接跳过
                            meaning = nestedMatch[2].trim();
                        } else {
                            break;
                        }
                    } else {
                ...
                // 回退到简单的匹配
                const simplePosMatch = explanation.match(/^([a-z.]+\.?)\s*(.*)$/i);
                if (simplePosMatch) {
                    partOfSpeech = simplePosMatch[1];
                    meaning = simplePosMatch[2];
                }
            }
        }
        
        // 最后清理：去掉意思里的冗余词性前缀
        const redundantPosPattern = /^[a-z.]+\s*(?:&|;|,)?\s*/i;
        while (meaning.match(redundantPosPattern)) {
            meaning = meaning.replace(redundantPosPattern, '');
        }
...
// 生成 vocabularyData.ts - 去掉 example 字段
